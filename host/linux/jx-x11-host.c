/* JX X11 desktop host prototype: jx.desktop/1
 *
 * Build (Debian/Ubuntu): cc -O2 -Wall -Wextra -o jx-x11-host jx-x11-host.c -lxcb
 *
 * This is deliberately small. It proves display ownership, input/event capture,
 * a painted root background, child launch, and basic top-level window management.
 * Canonical JX descriptors remain outside this file; a later transport layer
 * supplies Desktop/Icon/Shortcut descriptors and receives Bag snapshots.
 */
#include <xcb/xcb.h>
#include <stdio.h>
#include <stdlib.h>
...
static void flush_or_die(const char *what) {
    if (xcb_connection_has_error(conn)) {
        fprintf(stderr, "jx-x11-host: X connection failed during %s\n", what);
        exit(2);
    }
    xcb_flush(conn);
}
...
int main(int argc, char **argv) {
    ...
    if (xcb_connection_has_error(conn)) {
        fprintf(stderr, "jx-x11-host: cannot connect to X server\n");
        return 2;
    }
    ...
    if (nested) {
        /* Nested mode intentionally does not claim SubstructureRedirect on the
         * real root. A future Xephyr/root-window transport supplies the nested
         * pseudo-root. This switch is already reserved in the native host ABI. */
        fprintf(stderr, "jx-x11-host: --nested requires a nested X server such as Xephyr; using its root window\n");
    }
    ...
    if (error) {
        fprintf(stderr, "jx-x11-host: another window manager owns this display (X error %u)\n", error->error_code);
        free(error);
        xcb_disconnect(conn);
        return 3;
    }
    ...
    if (launch_program) {
        ...
        if (pid > 0) fprintf(stderr, "jx-x11-host: launched %s pid=%ld\n", launch_program, (long)pid);
    }

    fprintf(stderr, "jx-x11-host: desktop active %ux%u root=0x%08x\n",
