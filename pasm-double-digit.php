...
class PASM
{
    ...
    public static $pdb = 0;    // debug flag (DEFAULT FALSE)
    ...
    {    // Useful for some testing
        PASM::$chain[] = __FUNCTION__;// Will be easier to just play around
    ...
    {
        {
            PASM::$chain[PASM::$counter] = __FUNCTION__;
    ...
    {
        PASM::setup_chain(__FUNCTION__);
        PASM::$rdx = chr((PASM::$ecx + PASM::$ah)%256);

        if (PASM::$pdb === 1) { PASM::debug_loop_count(); }
        return new static;
    }
    ...
    {
        // Hot path: callers pass __FUNCTION__, which is already the short method name.
        // The old code exploded __METHOD__ and called func_get_args() on every instruction.
        // Chain replay only consumes PASM::$args for set(), and set() records its own
        // operands, so ordinary zero-argument instructions do not need an args entry.
        PASM::$chain[PASM::$counter++] = $method;
    }
    ...
    {     // run file on linux $ST0 is command and arguments are $string
        PASM::$chain[] = __FUNCTION__;// $rdx is the output file to show what happened.
    ...
    {     // same as above but pop
        PASM::$chain[] = __FUNCTION__;// again, $rdx is the output
        if (substr(php_uname(), 0, 7) == "Windows") {
