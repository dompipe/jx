...
require_once __DIR__ . '/pasm-loop-fuser.php';
...
function benchCompile(string $src, int $reps): array {
    $times=[]; $asm=''; $bc='';
    for ($i=0; $i<$reps; $i++) {
        $c = new PASMFusedCompiler(true, false, 8);
        $t=hrtime(true); $asm=$c->compile($src); $times[]=(hrtime(true)-$t)/1e6;
    }
    $c = new PASMFusedCompiler(true, false, 8); $bc=$c->compileToBytecode($src);
...
$out=['compiler'=>'active-loop-space-fused','n'=>$n,'reps'=>$reps,'cases'=>[]];
