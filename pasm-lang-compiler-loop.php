...
/**
 * PASL compiler with bounded out-of-line loop blocks.
 *
 * Canonical loop-space LCALL lowers to a direct branch with a fixed
 * continuation on the current PASM ISA. Native targets may emit a machine
 * call or inline the block. Canonical loop meaning remains unchanged.
 */
final class Compiler
{
    ...
    private PASMLoopSpace $loopSpace;
    /** @var list<array{label:string,lines:list<string>}> */
    private array $deferredBlocks = [];

    public function __construct(bool $optimize = true, bool $verbose = false, int $maxLoopDepth = PASMLoopSpace::DEFAULT_MAX_DEPTH)
    {
        $this->optimize = $optimize;
        $this->verbose = $verbose;
        $this->maxLoopDepth = $maxLoopDepth;
        $this->loopSpace = new PASMLoopSpace($maxLoopDepth);
    }
    ...
    public function compile(string $source): string
    {
        ...
        $this->emit('; loop lowering: bounded loop-space; LCALL => direct branch on PASM');
        $this->compileBlock($src);

        $ret = $this->pickReturnReg();
        $programEnd = $this->freshLabel('__jx_program_end');
        if ($this->deferredBlocks !== []) {
            $this->emit("        JMP   {$programEnd}");
            foreach ($this->deferredBlocks as $block) {
                $this->emit($block['label'] . ':');
                foreach ($block['lines'] as $line) $this->emit($line);
            }
            $this->emit("{$programEnd}:");
        }
        $this->emit("        RET    {$ret}");
    ...
    {
        return [
            'max_depth' => $this->loopSpace->maxDepth(),
            'compiled_blocks' => count($this->loopSpace->compiled()),
            'deferred_blocks' => count($this->deferredBlocks),
        ];
    }
    ...
    private function reset(): void
    {
        ...
        $this->loopStack = [];
        $this->complexVars = [];
        $this->loopSpace = new PASMLoopSpace($this->maxLoopDepth);
        $this->deferredBlocks = [];
    }
    ...
    private function compileBlock(string $src): void
    {
        ...
        while ($i < $n) {
            ...
            if (preg_match('/^foreach\s*\(/i', $rest)) throw new LangException('foreach requires collection lowering; loop-space supports it but PASL collection iteration is not linked yet', 'parse');
    ...
    /** Capture a block using the same register map, label sequence and loop-space. */
    private function captureBlock(string $source, ?array $loopContext = null): array
    {
        $outerLines = $this->lines;
        $this->lines = [];
        if ($loopContext !== null) $this->loopStack[] = $loopContext;
        try {
            $this->compileBlock($source);
            return $this->lines;
        } finally {
            if ($loopContext !== null) array_pop($this->loopStack);
            $captured = $this->lines;
            $this->lines = $outerLines;
            if (isset($captured)) {
                // return is handled above; finally only restores state.
            }
        }
    }

    private function captureStmt(string $statement): array
    {
        $outerLines = $this->lines;
        $this->lines = [];
        try {
            if (trim($statement) !== '') $this->compileStmt(trim($statement));
            return $this->lines;
        } finally {
            $this->lines = $outerLines;
        }
    ...
    private function defer(string $label, array $lines): void
    {
        $this->deferredBlocks[] = ['label' => $label, 'lines' => array_values($lines)];
    }

    private function enterLoop(string $kind, string $condition, string $body, ?string $init = null, ?string $step = null): PASMLoopBlock
    {
        try {
            return $this->loopSpace->enter($kind, $condition, $body, $init, $step);
        } catch (\InvalidArgumentException $e) {
            throw new LangException($e->getMessage(), 'loop-depth', null, $e);
        }
    }
    ...
    private function compileWhile(string $s): int
    {
        ...
        $loop = $this->enterLoop(PASMLoopKind::WHILE, trim($cond), $body);
        $Lh = $this->freshLabel('while_check');
        $Le = $this->freshLabel('while_end');

        $bodyLines = $this->captureBlock($body, ['break' => $Le, 'continue' => $Lh]);
        $bodyLines[] = "        JMP   {$Lh}";
        $this->defer($loop->bodySymbol, $bodyLines);

        $this->emit("; loop-space slot {$loop->slot} depth {$loop->depth}: while");
        $this->emit("{$Lh}:");
        $this->emit('; LCHECK + LCALL(body)');
        $this->condJump(trim($cond), $loop->bodySymbol, $Le);
        $this->emit("{$Le}:");

        $this->loopSpace->leave();
        return $ab;
    }
    ...
    private function compileFor(string $s): int
    {
        ...
        $loop = $this->enterLoop(PASMLoopKind::FOR, trim($cond), $body, trim($init), trim($step));
        $Lh = $this->freshLabel('for_check');
        $Le = $this->freshLabel('for_end');
        $continueTarget = $loop->stepSymbol ?? $Lh;

        $bodyLines = $this->captureBlock($body, ['break' => $Le, 'continue' => $continueTarget]);
        $bodyLines[] = '        JMP   ' . $continueTarget;
        $this->defer($loop->bodySymbol, $bodyLines);

        if ($loop->stepSymbol !== null) {
            $stepLines = $this->captureStmt(trim($step));
            $stepLines[] = "        JMP   {$Lh}";
            $this->defer($loop->stepSymbol, $stepLines);
        }

        $this->emit("; loop-space slot {$loop->slot} depth {$loop->depth}: for");
        $this->emit("{$Lh}:");
        if (trim($cond) === '') {
            $this->emit('; LCHECK elided (unconditional) + LCALL(body)');
            $this->emit("        JMP   {$loop->bodySymbol}");
        } else {
            $this->emit('; LCHECK + LCALL(body)');
            $this->condJump(trim($cond), $loop->bodySymbol, $Le);
        }
    ...
    private function compileStmt(string $stmt): void
    {
        ...
        // Record the one canonical mutation before lowering to the current ISA.
        try {
            $mutation = PASMVarOp::parse($stmt);
            $this->emit('; VAROP ' . $mutation['op'] . ' ' . $mutation['var']);
        ...
        if (preg_match('/^\$?([A-Za-z_]\w*)\s*=\s*(.+)$/', $stmt, $m)) {
            ...
            if ($this->optimize && preg_match('/^\$?([A-Za-z_]\w*)\s*\*\s*2$/', $rhs, $sm)) {
                $srcReg = $this->intReg($sm[1]);
                $this->emit("        ADD   {$dst} {$srcReg} {$srcReg}"); return;
            }
            if ($this->optimize && preg_match('/^\$?([A-Za-z_]\w*)\s*\+\s*0$/', $rhs, $sm)) {
                $srcReg = $this->intReg($sm[1]);
                if ($srcReg !== $dst) $this->emit("        MOVR  {$dst} {$srcReg}");
                return;
            }
