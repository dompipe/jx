...
int main(void){
    ...
    assert(jx_maintainer_runtime_add_source(&rt,&source)!=0);
    ...
    jx_maintainer_runtime_dispose(&rt); puts("jx-maintainer-runtime: unique provisioned source -> real live Bag commit through common composition"); return 0;
}
